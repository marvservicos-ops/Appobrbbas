...
2. **`/obras/[id]/financeiro`** (DRE por obra), componente `ObraPagamentos`:
   ...
   - **Aditivos de Contrato** (legado): conceito separado das OCs (um aditivo é uma alteração de escopo/valor do contrato original) — a tela já avisa que um aditivo também pode ser cadastrado como uma OC (usando o nome pra descrever do que se trata), que é o caminho recomendado daqui pra frente. O que já existe continua funcionando normalmente.
