...
import { Plus, Thermometer, User, Building2, CheckCircle2, XCircle, Calendar } from 'lucide-react'
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#E2E8F0]">
          <h2 className="font-syne font-semibold text-[#0F172A]">Novo Contrato de Manutenção</h2>
...
export default function ManutencoesPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#F8FAFC]">
      ...
      <main className="flex-1 p-4 md:p-6 max-w-7xl mx-auto w-full">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {filtrados.map(c => (
              <Link key={c.id} href={`/manutencoes/${c.id}`}
                className="card hover:shadow-md hover:border-[#4F7CFF]/30 border border-transparent transition-all cursor-pointer group">
      ...
      {showModal && <ModalNovoContrato onClose={() => setShowModal(false)} onSaved={() => { setShowModal(false); load() }} />}
    </div>
  )
}
